...
- **언어**: Go (표준 라이브러리만 사용: `net/http`, `encoding/json`, `flag`, `regexp`)
- **외부 API**: 네이버 뉴스 검색 API, Exa Contents API, Notion API
...
### `notion`
stdin으로 받은 `search` Markdown 출력을 파싱해 Notion 페이지를 생성합니다.

| 플래그 | 기본값 | 설명 |
|--------|--------|------|
| `--parent-id` | (필수) | 부모 페이지 ID |
| `--title` | (필수) | 새 페이지 제목 |
