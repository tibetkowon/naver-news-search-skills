...
```
1단계 (브라우징): search --highlights  → 여러 기사 제목·요약 빠르게 확인
...
노션에 저장할 때는 `--fetch`로 기사 전문을 가져와 에이전트가 직접 요약한 내용을 파이프합니다. `--highlights`는 Exa 자동 추출 특성상 노이즈가 섞일 수 있어 저장용으로 적합하지 않습니다.

```bash
# 권장: --fetch로 전문을 가져와 에이전트가 정리한 뒤 노션에 저장
{
  ...
  NAVER_CLIENT_ID=xxx NAVER_CLIENT_SECRET=yyy EXA_API_KEY=zzz \
    ./naver-news search --query "정치" --display 3 --fetch
} | NOTION_API_KEY=nnn ./naver-news notion \
    --parent-id <page_id> \
    --title "2026년 3월 2일 뉴스 브리핑"
```
...
| 사용자 요청 | 권장 커맨드 |
|------------|-------------|
| "오늘 IT 뉴스 어떤 거 있어?" | `search --query "IT" --sort date --display 10 --highlights` → 목록 훑기 |
| "인공지능 관련 최신 뉴스 요약해줘" | `search --query "인공지능" --sort date --display 5 --fetch` → 전문 가져와 에이전트가 요약 |
| "테슬라 뉴스 자세히 알고 싶어" | `search --query "테슬라" --display 3 --fetch` |
| "이 기사 내용 요약해줘: [URL]" | `fetch --url "[URL]"` |
| "반도체 산업 동향 분석해줘" | `search --query "반도체" --sort date --display 5 --fetch` |
| "카테고리별 오늘 뉴스 노션에 정리해줘" | 주제별 `search --fetch` → `notion` 파이프 |
| "오늘 뉴스 뭐뭐 있는지 간단히 알려줘" | 주제별 `search --highlights` → 에이전트가 목록 정리 |
